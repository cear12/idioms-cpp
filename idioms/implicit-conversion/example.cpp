...
struct A {
  ...
  operator int() const { return std::atoi(value_.c_str()); }

  void Print() const { std::cout << "A holds: " << value_ << "\n"; }
...
struct B {
  ...
  void Print() const { std::cout << "B holds: " << value_ << "\n"; }

 private:
  std::string value_;
};

void PrintInt(int x) { std::cout << "printInt: " << x << "\n"; }

void PrintDouble(double d) { std::cout << "printDouble: " << d << "\n"; }
