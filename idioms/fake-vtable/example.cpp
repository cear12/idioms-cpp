...
void BaseFoo() { std::cout << "Base::foo\n"; }

void DerivedFoo() { std::cout << "Derived::foo\n"; }

class Base {
 public:
  FakeVtable* vtable_;

  Base() { vtable_ = new FakeVtable{&BaseFoo}; }

  void Foo() {
    vtable_->foo_();  // dispatches through the fake vtable, not a real virtual
                      // call
  }

  virtual ~Base() { delete vtable_; }
};
...
class Derived : public Base {
 public:
  Derived() {
    vtable_->foo_ =
        &DerivedFoo;  // "override" by rewriting the function pointer
  }
};
