...
template <> struct KindOf<Circle>   { static constexpr ShapeKind kValue = ShapeKind::kCircle; };
template <> struct KindOf<Square>   { static constexpr ShapeKind kValue = ShapeKind::kSquare; };
template <> struct KindOf<Triangle> { static constexpr ShapeKind kValue = ShapeKind::kTriangle; };
...
int main() {
    ...
    for (Shape* s : shapes) {
        std::cout << s->ClassName()
                  << " is_a<Circle>=" << std::boolalpha << IsA<Circle>(*s) << "\n";
    }
